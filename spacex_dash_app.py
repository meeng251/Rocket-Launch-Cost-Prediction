...
space_df = pd.read_csv("spacex_launch_dash.csv")
max_payload = space_df["Payload Mass (kg)"].max()
...
def get_pie_chart(entered_site):
    if entered_site == "ALL":
        fig = px.pie(space_df, values="class", names="Launch SIte", title="Total Success Launches by Site")
    else:
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        filtered_df = space_df[(space_df['Payload Mass (kg)'] >= int(payload_range[0])) &
                                (space_df['Payload Mass (kg)'] <= int(payload_range[1]))
                               ]
        ...
        filtered_df = space_df[(space_df['Launch Site'] == entered_site) & 
                                (space_df['Payload Mass (kg)'] >= int(payload_range[0])) &
                                (space_df['Payload Mass (kg)'] <= int(payload_range[1]))
                               ]
